...
where (InvcHead.InvoiceDate >= '{selectedDate1}' and InvcHead.InvoiceDate <= '{selectedDate2}') AND [SalesRep].[RepReportsTo] = 'WEST' AND (Part.PartNum !='NULL') AND (Part.PartNum != 'NON COMPLIANCE') AND (Part.PartNum != 'VOLUME REBATE') AND (Part.PartNum != 'CO-OP')
