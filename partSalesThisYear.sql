...
where ((MONTH(InvcHead.InvoiceDate) < MONTH(GETDATE())) or (MONTH(InvcHead.InvoiceDate) = MONTH(GETDATE())) and DAY(InvcHead.InvoiceDate) <= DAY(GETDATE())) and (YEAR(InvcHead.InvoiceDate) = YEAR(GETDATE()))
