...
where (MONTH(InvcHead.InvoiceDate) >= '10') and (MONTH(InvcHead.InvoiceDate) < '13') and (YEAR(InvcHead.InvoiceDate) = YEAR(GETDATE())) AND SalesRep.RepReportsTo = 'EAST' AND (Part.PartNum !='NULL') AND (Part.PartNum != 'NON COMPLIANCE') AND (Part.PartNum != 'VOLUME REBATE') AND (Part.PartNum != 'CO-OP')