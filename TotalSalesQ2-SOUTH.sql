...
where (MONTH(InvcHead.InvoiceDate) >= '4') and (MONTH(InvcHead.InvoiceDate) < '7') and (YEAR(InvcHead.InvoiceDate) = YEAR(GETDATE())) AND ([SalesRep].[RepReportsTo] = 'SOUTHEAS' OR [SalesRep].[RepReportsTo] = 'SOUTHWES') AND (Part.PartNum !='NULL') AND (Part.PartNum != 'NON COMPLIANCE') AND (Part.PartNum != 'VOLUME REBATE') AND (Part.PartNum != 'CO-OP')