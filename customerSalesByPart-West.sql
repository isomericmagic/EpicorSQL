...
where (InvcHead.InvoiceDate >= '{date1}' and InvcHead.InvoiceDate <= '{date2}') AND [Customer].[CustNum] = '{custnum}' AND [SalesRep].[RepReportsTo] = 'WEST' AND (Part.PartNum !='NULL') AND (Part.PartNum != 'NON COMPLIANCE') AND (Part.PartNum != 'VOLUME REBATE') AND (Part.PartNum != 'CO-OP')
