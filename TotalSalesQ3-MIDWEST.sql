...
where (MONTH(InvcHead.InvoiceDate) >= '7') and (MONTH(InvcHead.InvoiceDate) < '10') and (YEAR(InvcHead.InvoiceDate) = YEAR(GETDATE())) AND SalesRep.RepReportsTo = 'MIDWEST' AND (Part.PartNum !='NULL') AND (Part.PartNum != 'NON COMPLIANCE') AND (Part.PartNum != 'VOLUME REBATE') AND (Part.PartNum != 'CO-OP')